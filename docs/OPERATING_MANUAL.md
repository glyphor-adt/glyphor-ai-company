...
calculate_roi(company_data, product)        — internal engine ROI model for prospect
...
### Skills
- Output quality auditing — grading web builds on a scale from F (AI smell) to A+ (agency-grade)
...
get_design_quality_summary(days)             — web build output quality grades and trends
...
GREEN (autonomous):
   ...
   - Grade web builds based on quality data
...
query_revenue_by_product(period)          — web build vs Pulse breakdown
