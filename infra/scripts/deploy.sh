...
gcloud run deploy glyphor-scheduler \
  ...
  --min-instances=1 \
  --max-instances=3 \
  --set-env-vars="NODE_ENV=production,AGENT365_ENABLED=true,AGENT_TRACING_ENABLED=${AGENT_TRACING_ENABLED},WORKER_URL=${WORKER_URL},WORKER_REQUEST_TIMEOUT_MS=900000" \
