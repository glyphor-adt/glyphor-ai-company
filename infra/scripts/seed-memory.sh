...
curl -s -X POST "${SUPABASE_URL}/rest/v1/company_agents" \
  ...
  -H "Prefer: resolution=merge-duplicates" \
  -d '[
    {"role": "chief-of-staff", "name": "Chief of Staff", "status": "active", "model": "gemini-2.5-flash", "schedule": "morning briefings 7:00/7:30 CT + on-demand"},
    {"role": "cto", "name": "CTO", "status": "stub", "model": "gemini-2.5-flash", "schedule": "every 30 min health check + on-demand"},
    {"role": "cfo", "name": "CFO", "status": "stub", "model": "gemini-2.5-flash", "schedule": "daily 9:00 CT cost check"},
    {"role": "cpo", "name": "CPO", "status": "stub", "model": "gemini-2.5-flash", "schedule": "weekly Monday 10:00 CT usage analysis"},
    {"role": "cmo", "name": "CMO", "status": "stub", "model": "gemini-2.5-flash", "schedule": "weekly Monday 9:00 CT content planning"},
    {"role": "vp-customer-success", "name": "VP Customer Success", "status": "stub", "model": "gemini-2.5-flash", "schedule": "daily 8:00 CT health scoring"},
    {"role": "vp-sales", "name": "VP Sales", "status": "stub", "model": "gemini-2.5-flash", "schedule": "Mon/Thu 9:00 CT pipeline review"}
  ]'
