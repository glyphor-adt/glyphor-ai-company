-- Sync Marketing skill playbooks from markdown source files.
-- Sources:
--   skills/marketing/content-creation.md       (v1 → v3)
--   skills/marketing/seo-optimization.md       (v1 → v2)
--   skills/marketing/social-media-management.md (v1 → v2)
...
| Tool | When to use it |
|------|---------------|
| pulse_create_storyboard_from_idea | Idea to screenplay to scene breakdown. The starting point for any video content. |
...
| Tool | When to use it |
|------|---------------|
| pulse_create_hero_promo | Full pipeline: idea to storyboard to scenes to video to audio. One call = complete promo. Product launches, feature announcements. |
...
### Brand Intelligence and Distribution (4 tools)
...
| pulse_list_brand_kits | Access saved brand kits — Glyphor tokens should be loaded for consistent generation. |
...
4. Hero image — pulse_enhance_prompt then pulse_generate_concept_image then pulse_upscale_image
5. In-article graphics — pulse_generate_concept_image for diagrams, pulse_product_recontext for product shots in context
6. Review — submit_content_for_review then validate_brand_compliance
...
### Video Promo (storyboard to produce to finish)

1. Concept — message, format, duration, platform
2. Storyboard — pulse_create_storyboard_from_idea then pulse_suggest_scenes for gaps then pulse_generate_scene_images to preview
3. Script — pulse_generate_storyboard_script then pulse_generate_voiceover_script
...
5. Audio — pulse_text_to_speech for narration then pulse_generate_music for background then pulse_generate_sound_effect for transitions
6. Polish — pulse_kling_video_extend if too short then pulse_remix_video for variants
7. Distribute — pulse_create_share_link for review then publish

### Social Campaign (multi-format, multi-channel)

1. Campaign brief — message, audience, platforms, timeline
2. Hero asset — pulse_enhance_prompt then pulse_generate_concept_image
3. Variant assets — pulse_generate_promo_scenes from hero produces a visual family
4. Video variant — pulse_kling_image_to_video to animate hero then create social clip
...
2. Header image — pulse_generate_concept_image then pulse_expand_image to email banner ratio
...
6. Draft — draft_email then submit_content_for_review
...
2. Hero promo video — pulse_create_storyboard_from_idea then pulse_create_hero_promo
3. Voiceover — pulse_generate_voiceover_script then pulse_text_to_speech
...
2. Write — draft_case_study (Problem then Approach then Result then Quote)
...
Blog: Hook (surprising fact/result) then Thesis then Evidence (2-4 sections) then "So what?" then CTA

Case study: Problem (specific, quantified) then Approach (technically credible) then Result (numbers) then Quote

Email: Subject (<50 chars) then Opening (not "I hope this finds you well") then Core (one idea) then CTA (single)

LinkedIn: Hook in first line then Insight/data then CTA or question then 2-3 hashtags
...
Build a pattern library: after 3 months, you should know which content types drive engagement, which Pulse tools produce the best brand-native visuals, and which production pipelines are most efficient. This data turns content creation from guessing into a system.
      ...
      ARRAY[
        ...
        'pulse_generate_concept_image', 'pulse_edit_image', 'pulse_enhance_prompt',
        'pulse_enhance_video_prompt', 'pulse_polish_scene_prompt',
        'pulse_remove_background', 'pulse_upscale_image', 'pulse_expand_image',
        'pulse_extract_image_text', 'pulse_replace_image_text',
        'pulse_transform_viral_image', 'pulse_product_recontext', 'pulse_doodle_to_image',
        'pulse_generate_video', 'pulse_poll_video_status', 'pulse_list_videos',
        'pulse_remix_video',
        'pulse_text_to_speech', 'pulse_generate_sound_effect', 'pulse_generate_music',
        'pulse_create_storyboard_from_idea', 'pulse_list_storyboards',
        'pulse_get_storyboard', 'pulse_generate_scene_images', 'pulse_suggest_scenes',
        'pulse_storyboard_chat', 'pulse_generate_storyboard_script',
        'pulse_generate_voiceover_script',
        'pulse_kling_text_to_video', 'pulse_kling_image_to_video',
        'pulse_kling_video_extend', 'pulse_kling_video_reference',
        'pulse_kling_multi_shot', 'pulse_poll_multi_shot', 'pulse_kling_poll_task',
        'pulse_kling_lip_sync', 'pulse_kling_motion_upload', 'pulse_kling_motion_create',
        'pulse_kling_create_voice',
        'pulse_create_hero_promo', 'pulse_create_multi_angle',
        'pulse_create_product_showcase', 'pulse_generate_promo_scenes',
        'pulse_analyze_brand_website', 'pulse_analyze_image_for_video',
        'pulse_create_share_link', 'pulse_list_brand_kits'
      ]::text[],
      3
    ),
    (
...
1. Relevance — does this keyword match what Glyphor actually does?
2. Intent — is the searcher looking to learn, compare, or buy?
3. Difficulty — can we realistically rank in 6 months?
...
Title tag — the single most important on-page factor. Include the primary keyword near the beginning. Under 60 characters. Compelling enough to earn the click.

Meta description — under 155 characters. Include the primary keyword. Clear value proposition. End with a call to action when appropriate.

Heading structure — H1 through H4 should form a logical outline. One H1 per page. H2s for major sections with secondary keywords. Never skip levels.

Content quality signals:
- Word count appropriate to the topic (guides: 2000-4000 words; product pages: 500-1000; blog posts: 1200-2500)
- Primary keyword in the first 100 words
- Related keywords and synonyms appear naturally throughout
...
- No keyword stuffing — density above 2-3% is probably over-optimized
...
Indexing: get_indexing_status to check which pages are indexed. submit_sitemap to ensure the sitemap is current. Watch for accidental noindex tags, blocked resources in robots.txt, or canonical tag errors.

Page speed: Core Web Vitals matter for ranking. LCP under 2.5s, CLS under 0.1, FID/INP under 200ms.

Mobile: Google uses mobile-first indexing. All marketing pages must be responsive.

Structured data: Blog posts should have Article schema. Product pages should have Product or SoftwareApplication schema. FAQ sections should have FAQPage schema.

### Backlink Analysis

Use get_backlink_profile and query_backlinks to monitor:

- Total backlinks and referring domains — trend over time
- Link quality — a single link from TechCrunch is worth more than 100 from random directories
- Anchor text distribution — should be natural, not over-optimized
- Toxic links — flag for disavow if needed
- Competitor backlinks — query_competitor_rankings reveals potential outreach targets

### Google Search Console

Use query_search_console and get_search_performance regularly.
...
1. Lisa identifies keyword opportunities and sends target keywords to Tyler via send_agent_message
...
This loop should run continuously. Content written 6 months ago and never revisited is decaying in rankings. Refresh old content with updated data, new internal links, and improved keyword targeting.
      $seo_optimization$,
      ARRAY[
        'web_search', 'web_fetch', 'save_memory', 'send_agent_message',
        'discover_keywords', 'track_keyword_rankings', 'query_keyword_data',
        'query_seo_rankings', 'get_seo_data', 'update_seo_data',
        'get_search_performance', 'query_search_console',
        'get_indexing_status', 'submit_sitemap',
        'analyze_page_seo', 'analyze_content_seo',
        'get_backlink_profile', 'query_backlinks', 'query_competitor_rankings',
        'get_content_metrics', 'query_content_performance'
      ]::text[],
      2
    ),
    (
      ...
      'social-media-management',
      'marketing',
      'Own Glyphor''s LinkedIn and X presence — scheduling, engagement, community building, and social analytics. Use when drafting social posts, scheduling content, monitoring brand mentions, responding to engagement, running social campaigns, or producing social performance reports.',
      $social_media$
# Social Media Management

Your job is not to "post on social." Your job is to build Glyphor''s presence on LinkedIn and X into a credible, authoritative, engaging voice in the autonomous AI space. Every post either builds trust, demonstrates capability, or starts a conversation worth having.

## Platform Strategy

### LinkedIn: The Professional Stage

LinkedIn is where we reach operators — CTOs, VP Eng, AI leads, founders — the people who would buy or champion Glyphor. The content here is longer-form, thoughtful, data-backed, and professionally authoritative.

**What works on LinkedIn:**
- Operational insights from running an AI company. Not "AI can help businesses" but "our QA agent caught 23 issues this week that our human reviewers missed. Here is how we built the review loop."
- Framework posts. "The 3 metrics that actually matter when measuring AI agent autonomy" with real numbers from Glyphor''s operation.
- Behind-the-scenes. How we handle agent failures, how we design escalation, what our morning briefings look like. Transparency builds trust.
- Contrarian takes with evidence. "Most AI agent products are wrappers around chat completions. Here is why that doesn''t work for production operations."

**LinkedIn mechanics:**
- Post length: 150-300 words per post. Enough to deliver value, short enough to keep attention.
- First line is everything — it must hook. One sentence that creates curiosity or says something unexpected.
- Use line breaks liberally. Dense paragraphs die on LinkedIn.
- End with a question or CTA that invites engagement.
- Post timing: Tuesday-Thursday, 8-10 AM CT for primary audience overlap.

### X (Twitter): The Technical Watercooler

X is more casual, faster, and technical. The audience skews developer and early-adopter. Threads let us go deep; single tweets let us stay in the conversation.

**What works on X:**
- Technical micro-insights. Short, specific observations from running an AI agent operation.
...
- Real-time commentary. When AI news breaks, our perspective matters. Not reposting — adding our angle.
- Tool/approach sharing. Developers love seeing how things work.
...
- Quote-tweeting competitors or industry figures with our perspective (constructive takes, not attacks)

## The Content Approval Flow

All social content is approval-gated. This is a pipeline, not a bottleneck.

Draft then submit_content_for_review then Maya (CMO) reviews:
  - approve_content_draft then schedule_social_post (with optimal timing)
  - reject_content_draft (with specific feedback) then revise then resubmit

For time-sensitive content (breaking news, trending conversations), flag urgency to Maya via send_agent_message.

## Scheduling and Calendar

Use get_content_calendar to see what is planned. Social should complement blog publishing, product milestones, and company milestones.
...
Social is how we build relationships with journalists, analysts, developers, and potential customers before we ever need anything from them.

## Analytics and Reporting

Weekly social report for CMO using get_social_metrics, query_social_metrics, get_post_performance, query_post_performance, and get_social_audience:
...
- Competitor activity: what competitors posted that performed well
...
Save weekly reports as memories. The patterns over months reveal what truly works vs. what seemed to work once.
      ...
      ARRAY[
        'web_search', 'web_fetch', 'save_memory', 'send_agent_message',
        'draft_social_post', 'schedule_social_post', 'get_social_metrics',
        'query_social_metrics', 'get_post_performance', 'query_post_performance',
        'get_social_audience', 'query_audience_demographics',
        'monitor_mentions', 'reply_to_social',
        'get_content_calendar', 'get_trending_topics',
        'submit_content_for_review', 'approve_content_draft', 'reject_content_draft',
        'get_content_metrics', 'query_content_performance'
      ]::text[],
      2
...
-- Refresh agent_skills holders for marketing skills
-- First remove stale assignments for these skills
...
-- Refresh task_skill_map for marketing skills
WITH mapping_payload AS (
  SELECT *
  FROM (VALUES
    ('(?i)(content creat|blog post|blog draft|case study|email campaign|video promo|social campaign|product announcement|write content|content calendar|pulse|storyboard|hero image)', 'content-creation', 18),
    ('(?i)(seo|search engine|keyword|ranking|backlink|search console|indexing|sitemap|on.page optimization|meta description|page speed)', 'seo-optimization', 17),
    ('(?i)(social media|linkedin post|twitter|tweet|social post|social schedule|social metric|mention monitor|social engagement|community)', 'social-media-management', 17)
