/**
 * Organizational & Talent Analyst (Marcus Chen) — Runner
 * Reports to Sophia Lin (VP of Research & Intelligence). Talent & org research.
 */
import {
...
import { ORG_ANALYST_SYSTEM_PROMPT } from './systemPrompt.js';
import { createOrgAnalystTools } from './tools.js';
...
export async function runOrgAnalyst(params: OrgAnalystRunParams = {}) {
  ...
  const config: AgentConfig = {
    id: `marcus-chen-${task}-${today}-${Date.now()}`,
    role: 'org-analyst',
    systemPrompt: ORG_ANALYST_SYSTEM_PROMPT,
