...
export function createSocialMediaTools(): ToolDefinition[] {
  return [
    ...
    /* ── get_scheduled_posts ───────────── */
    {
      ...
      description:
        'List all scheduled social media posts. Supports filtering by platform, status, date range, and limit.',
      parameters: {
        platform: {
          type: 'string',
          description: 'Filter by platform (linkedin, twitter, instagram).',
        },
        status: {
          type: 'string',
        ...
        limit: {
          type: 'number',
          description: 'Max number of posts to return (default: 50).',
        },
      },
      execute: async (params): Promise<ToolResult> => {
        ...
        try {
          ...
          const rows = await systemQuery(
            `SELECT id, platform, text, media_url, scheduled_at, status, api_status, created_at
          ...
          return {
            success: false,
            error: `Failed to fetch scheduled posts: ${err instanceof Error ? err.message : String(err)}`,
          };
        }
      },
    },
    ...
    /* ── get_social_metrics ────────────── */
    {
      ...
      description:
        'Read social media metrics including followers, engagement rate, reach, impressions, and clicks. ' +
        'Supports filtering by platform and date range.',
      parameters: {
        platform: {
          type: 'string',
          description: 'Filter metrics by platform (linkedin, twitter, instagram).',
        },
        date_from: {
          type: 'string',
          description: 'Start date filter (ISO 8601).',
        ...
        try {
          ...
          const rows = await systemQuery<{
            ...
            `SELECT platform,
             ...
             FROM social_metrics ${where}
             GROUP BY platform`,
            values,
          );

          return { success: true, data: { metrics: rows } };
        } catch (err) {
          return {
            success: false,
            error: `Failed to fetch social metrics: ${err instanceof Error ? err.message : String(err)}`,
          };
        }
      },
    },
