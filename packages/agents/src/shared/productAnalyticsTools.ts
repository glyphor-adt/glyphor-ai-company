...
export function createProductAnalyticsTools(): ToolDefinition[] {
  return [
    /* ── query_analytics_events ────────────── */
    {
      ...
      description:
        'Query the analytics_events table. Returns event counts, trends, and breakdowns ' +
        'filtered by event type, channel, plan, date range, and optional user.',
      parameters: {
        event_type: {
          type: 'string',
          description: 'Type of event to filter (e.g. "page_view", "click", "signup").',
          required: true,
        },
        channel: {
          type: 'string',
          description: 'Acquisition or traffic channel (e.g. "organic", "paid", "referral").',
          required: true,
        },
        plan: {
          type: 'string',
          description: 'Subscription plan to filter by (e.g. "free", "pro", "enterprise").',
          required: true,
        },
        date_range: {
          type: 'string',
        ...
        user_id: {
          type: 'string',
          description: 'Optional user ID to narrow results to a single user.',
        },
      },
      async execute(params): Promise<ToolResult> {
        ...
        try {
          const conditions = [
            ...
            'channel = $2',
            'plan = $3',
            "created_at >= NOW() - CAST($4 AS INTERVAL)",
          ];
          const values: unknown[] = [eventType, channel, plan, dateRange];
          ...
          const trends = await systemQuery(
          ...
          const breakdowns = await systemQuery(
            `SELECT event_type,
                    channel,
                    plan,
                    COUNT(*) AS count
             FROM analytics_events
             WHERE ${where}
             GROUP BY event_type, channel, plan
             ORDER BY count DESC`,
            values,
          );
          ...
          return {
            success: false,
            error: `Analytics events query failed: ${err instanceof Error ? err.message : String(err)}`,
          };
        }
      },
    },
