      ...
      execute: async (params): Promise<ToolResult> => {
        try {
          ...
          const res = await fetch(`${serviceUrl}/pdf`, {
            ...
            body: JSON.stringify({ html, format: params.format || 'A4', landscape: params.landscape ?? false }),
            signal: AbortSignal.timeout(60_000),
          });

          if (!res.ok) {
            return { success: false, error: `PDF service returned ${res.status}: ${await res.text()}` };
          }
          ...
          return { success: false, error: `generate_pdf failed: ${(err as Error).message}` };
        }
      },
    },
  ];
