...
export function buildVisualPrompt(record: AnalysisRecord): string {
  ...
  return [
    ...
    ``,
    `Color palette: primary cyan (#00E0FF), white (#FFFFFF) background, dark charcoal (#1A1A2E) text, emerald (#34D399) for positive, rose (#FB7185) for negative, amber (#FBBF24) for caution. Use soft pastel tinted backgrounds for card sections. Use thin black (#1A1A2E) borders on all cards and sections. Footer bar should be black with white text.`,
    ``,
    `LAYOUT (3 rows):`,
    ``,
    `ROW 1 â€” Header banner (8% height):`,
    `Full-width solid black (#000000) bar. Large bold white sans-serif title: "${title.toUpperCase()}", left-aligned with comfortable padding. Leave the top-right corner empty — a logo will be composited there after generation. Do NOT render any icon, logo, wordmark, subtitle, date, or secondary text in the header.`,
    ``,
...
export function buildStrategyLabVisualPrompt(record: StrategyAnalysisRecord): string {
  ...
  return [
    ...
    '',
    `White background, modern flat design, corporate sans-serif typography, and executive-ready slide composition. Use ${accentColor} as the primary accent color, with light gray and soft teal secondary accents. Use thin black (#1A1A2E) borders on all cards and sections. Footer bar should be black with white text. Keep the visual clean, data-rich, and highly legible, with rounded rectangles, thin dividers, subtle drop shadows, and generous whitespace.`,
    '',
    `Header strip: a full-width solid black (#000000) bar spanning the top of the image, approximately 8% of image height. Inside it, render the report title "${displayTitle}" in bold white sans-serif text, left-aligned with comfortable padding. Leave the top-right corner of the header empty — a logo will be composited there after generation. Do NOT render any icon, logo, wordmark, company name, date badge, or secondary text in the header. Just the title in white on the black bar.`,
    '',
    ...
    '- Follow the layout literally: black header bar with white title, metric band, left middle section, right middle section, bottom-left panel, bottom-right panel, footer zone',
