...
export async function queryOpenAICosts(adminKey: string, days = 30): Promise<OpenAICostBucket[]> {
...
export async function syncOpenAIBilling(
  ...
  const rows: Array<{
    ...
    service: string;
    cost_usd: number;
    usage: { date: string; currency: string; object: string };
    product: string;
    recorded_at: string;
  }> = [];
  ...
  for (const bucket of buckets) {
    ...
    for (const result of bucket.results) {
      ...
      if (!costValue || Number.isNaN(costValue)) continue;
      ...
      dailyTotals.set(date, (dailyTotals.get(date) ?? 0) + costValue);
    }
  }
  ...
  if (rows.length > 0) {
    ...
    for (const date of uniqueDates) {
      await systemQuery("DELETE FROM api_billing WHERE provider = $1 AND usage->>'date' = $2", ['openai', date]);
    }
