...
export function Card({
  ...
  return (
    <div
      className={`glass-card glass-panel panel-primary ${interactive ? GLASS_CARD_INTERACTIVE : GLASS_CARD_BASE} p-5 ${interactive ? 'glass-card--interactive' : ''} ${glow ? 'glass-card--glow' : ''} ${className}`}
...
export function InnerCard({
  ...
  return (
    <div
      className={`glass-raised glass-panel panel-nested glass-inner-card rounded-xl border border-primary/20 px-4 py-3 ${className}`}
      style={buildCardStyle(accent, style)}
      {...rest}
    >
      <div className="glass-card__content">
