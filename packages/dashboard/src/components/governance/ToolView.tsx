              ...
              const tone = tool.severity === 'critical'
                ...
                : tool.severity === 'high'
                  ...
                  : tool.severity === 'medium'
                    ...
                    : '';
              return (
                <tr key={tool.id} className={`border-b border-primary/15 align-top ${tone}`}>
                  <td className="py-3 pr-3">
                    ...
                    <p className="mt-1 flex items-center gap-1.5 text-[11px] text-txt-muted">
                      {(() => {
                        ...
                        return (
                          <span className={`badge badge-xs badge-up ${pm.badge}`}>
                            {pm.label}
                          </span>
                        );
                      })()}
                  ...
                  <td className="py-3 pr-3">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {staleDays != null && staleDays > 7 && (
                        <span className="rounded-lg badge badge-amber px-2 py-0.5 text-[11px]">
                          {staleDays}d stale
                        </span>
                      )}
                    </div>
  ...
  return (
    <Card>
      ...
      <div className="space-y-3">
        {items.map((item) => (
          <div key={item.toolName} className="flex flex-wrap items-center justify-between gap-3 rounded-xl theme-glass-panel p-4">
            <div>
            ...
            <>
              ...
              {results.slice(0, 12).map((result) => {
                ...
                return (
                  <div key={result.key} className="rounded-xl theme-glass-panel p-4">
                    <div className="flex items-center gap-2">
                      <span className={`badge badge-sm badge-up ${toolPm.badge}`}>
                        {toolPm.label}
                      </span>
                      <p className="text-sm font-semibold text-txt-primary">{result.label}</p>
                    </div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {result.agents.map((role) => (
                        <div
                          key={role}
                          className="rounded-lg theme-glass-panel-soft px-2.5 py-1.5 text-[11px]"
                        >
