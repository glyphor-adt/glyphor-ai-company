-- ============================================================
-- KNOWLEDGE SEED — Run in Supabase SQL Editor
-- Populates all agent knowledge: knowledge base, bulletins,
-- company pulse, knowledge graph, and first directives.
-- ============================================================
...
-- ════════════════════════════════════════════════════════════
-- 3. COMPANY PULSE (singleton heartbeat row)
-- ════════════════════════════════════════════════════════════

INSERT INTO company_pulse (
  id, mrr, active_users, platform_status, company_mood, highlights, updated_at
) VALUES (
  'current', 0, 2, 'degraded', 'building',
  '[
...
-- ════════════════════════════════════════════════════════════
-- NOTE: Embeddings should be generated afterward via a one-time
-- script using EmbeddingClient.embed() on each node's content.

INSERT INTO kg_nodes (node_type, title, content, department, importance, tags, created_by, status) VALUES

-- Products
('product', 'Pulse',
'Glyphor creative production studio. PRIORITY ONE. AI-powered image generation (Imagen 4), video generation (Kling, Veo, Sora, Runway), multi-track video editing, storyboarding with AI narrative agents, audio suite (ElevenLabs), brand analysis, campaign launcher, avatar system. 100+ edge functions, 80+ hooks. Deployed at pulse.glyphor.ai on Supabase + Vercel. Stripe payments built. ~80-90% complete, needs audit before Product Hunt launch.',
'product', 1.0, ARRAY['pulse','product','priority-one','creative','launch','B2C'], 'system', 'active'),

('product', 'Fuse',
'Glyphor AI development platform. PRIORITY TWO. Transforms prompts into deployed React/Next.js apps via squad of build agents (UX Engineer, Build Verifier, Developer, Publisher, Deployment Sentinel). Three tiers: Prototype (<60s), Full Build (2-5min), Iterate (30-90s). Persistent intelligence with plan caching, graph memory, critique-revise loops. Deployed at fuse.glyphor.ai on GCP Cloud Run + Supabase + Firebase. NEVER call it "vibe coding."',
'product', 0.8, ARRAY['fuse','product','priority-two','development','agents'], 'system', 'active'),

('product', 'Reve',
'AI-powered virtual try-on fashion app. FUTURE — domain only, no development. Zero resource allocation.',
'product', 0.2, ARRAY['reve','future','inactive'], 'system', 'active'),

-- Market
('concept', 'Target Customer - Pulse',
'Individual content creators and influencers who need consistent, professional visual and video content. B2C/prosumer model. Currently use Midjourney ($10-30/mo), Canva ($13/mo), Runway ($12-76/mo), or combinations. Pain: too many tools, inconsistent results, no end-to-end workflow. Pulse is the single-platform solution.',
'marketing', 0.9, ARRAY['market','pulse','creators','B2C','ICP'], 'system', 'active'),

('concept', 'Product Hunt Launch',
'Primary GTM event for Pulse. Requirements: polished product, PH page (tagline ≤60 chars, screenshots, demo video, maker story), launch day social across @glyphor.ai (FB, LinkedIn, Instagram, TikTok), community engagement, first-100-users incentive. Best on Tuesday-Thursday. First hour critical for ranking.',
'marketing', 1.0, ARRAY['launch','product-hunt','GTM','pulse'], 'system', 'active'),

-- Financial
('metric', 'Revenue Target',
'$1M revenue in first 12 months post-launch. At $15-50/month consumer pricing, requires 1,700-5,500+ paying users. Bootstrapped organic growth only — no paid acquisition.',
'finance', 0.9, ARRAY['revenue','target','pricing'], 'system', 'active'),

('metric', 'Monthly Burn',
'~$800/mo infrastructure. GCP $187, Gemini API $412, Supabase $125, Vercel $67. Offset by $2K/mo founder contributions. Net +$1,200/mo surplus.',
'finance', 0.8, ARRAY['burn','cost','budget'], 'system', 'active'),

-- Infrastructure
('concept', 'Three Infrastructure Stacks',
'Glyphor runs three separate systems: (1) Pulse on Supabase + Vercel + R2, (2) Fuse on GCP Cloud Run + Supabase + Firebase + GCS + Cloudflare Workers, (3) Agent Platform on GCP Cloud Run + Supabase + Pub/Sub. Different databases, hosting, AI providers. Must monitor, cost-track, incident-manage independently.',
'engineering', 0.9, ARRAY['infrastructure','architecture','monitoring'], 'system', 'active'),

-- Competitive
('concept', 'Midjourney',
'Primary Pulse competitor. Best image generation quality. Discord-only (friction). No editing/video/audio/storyboards/campaigns. $10-30/mo. Our edge: full production studio vs single generator.',
'marketing', 0.7, ARRAY['competitor','midjourney','pulse'], 'system', 'active'),

('concept', 'Canva',
'Indirect Pulse competitor. Excellent templates. AI is secondary add-on. $13/mo. Our edge: AI-native, generation is core, more powerful models, unified workflow.',
'marketing', 0.7, ARRAY['competitor','canva','pulse'], 'system', 'active'),

('concept', 'Runway',
'Video-focused Pulse competitor. Gen-3 model. $12-76/mo. Our edge: video + image + audio + editing + storyboards + brand tools. Complete vs video-only.',
'marketing', 0.7, ARRAY['competitor','runway','pulse'], 'system', 'active'),

('concept', 'Devin',
'Primary Fuse competitor. Single AI agent, black box, ~$500/mo. Our edge: transparent squad pipeline, three execution tiers, persistent learning, observable.',
'product', 0.6, ARRAY['competitor','devin','fuse'], 'system', 'active'),

-- Social
('concept', 'Social Media Presence',
'All accounts: @glyphor.ai on Facebook, LinkedIn, Instagram, TikTok. Maya has Meta Business Platform access. Pre-launch content should build audience toward Product Hunt launch.',
'marketing', 0.8, ARRAY['social','marketing','channels'], 'system', 'active'),

-- Risks
('risk', 'Telemetry Blackout',
'Agent platform reporting $0 costs, 0% build success, Teams HTTP 400. Likely Pub/Sub OIDC token issue (fix identified). P0 for Marcus on agent platform.',
'engineering', 0.9, ARRAY['incident','telemetry','p0'], 'system', 'active'),

('risk', 'Crowded Market Launch',
'Pulse launching into competitive creative AI space. Differentiation is workflow + full production pipeline, not raw generation quality. Product Hunt is one-shot — must be polished. No paid acquisition fallback.',
'operations', 0.8, ARRAY['risk','launch','competition'], 'system', 'active'),

-- Opportunities
('opportunity', 'Creator Economy Distribution',
'Creators share tools with audiences. One influential creator using Pulse visibly could drive thousands of signups. Maya: identify creator partnerships. Kai: social content showing Pulse outputs.',
'marketing', 0.8, ARRAY['growth','creators','viral','distribution'], 'system', 'active'),

('opportunity', 'Eaton Corporation (Context)',
'Fortune 500 manufacturer. Active AI transformation. Met Feb 20, 2026. CONTEXT ONLY — Microsoft customer engagement, not a Glyphor prospect currently. May become relevant for Fuse enterprise positioning in future.',
'sales', 0.3, ARRAY['eaton','context','future','microsoft'], 'system', 'active');
...
SELECT s.id, t.id, 'enables', 1.0, 1.0,
  'Pulse is the lead product driving first revenue', 'system'
FROM kg_nodes s, kg_nodes t
WHERE s.title = 'Pulse' AND t.title = 'Revenue Target';

INSERT INTO kg_edges (source_id, target_id, edge_type, strength, confidence, evidence, created_by)
SELECT s.id, t.id, 'depends_on', 1.0, 1.0,
  'Product Hunt is the launch channel for Pulse', 'system'
FROM kg_nodes s, kg_nodes t
WHERE s.title = 'Product Hunt Launch' AND t.title = 'Pulse';

INSERT INTO kg_edges (source_id, target_id, edge_type, strength, confidence, evidence, created_by)
SELECT s.id, t.id, 'blocks', 0.8, 0.9,
  'Crowded market increases need for differentiated positioning', 'system'
FROM kg_nodes s, kg_nodes t
WHERE s.title = 'Crowded Market Launch' AND t.title = 'Revenue Target';


-- ════════════════════════════════════════════════════════════
-- 6. FOUNDER DIRECTIVES (first work assignments)
-- ════════════════════════════════════════════════════════════
