...
(async () => {
  ...
  try {
    ...
    const missionBefore = await client.query(`
      SELECT id, section, LEFT(content, 300) AS snippet, updated_at
      FROM company_knowledge_base
      WHERE section = 'mission'
      ORDER BY updated_at NULLS LAST, id;
    `);
    console.table(missionBefore.rows);

    await client.query('BEGIN');

    await client.query(`
      DELETE FROM company_knowledge_base
      WHERE section IN ('operating_doctrine', 'standing_orders', 'mission');
    `);

    await client.query(
      `INSERT INTO company_knowledge_base (section, audience, content) VALUES ($1, $2, $3)`,
      ['mission', 'all', missionContent],
    );
    await client.query(
      `INSERT INTO company_knowledge_base (section, audience, content) VALUES ($1, $2, $3)`,
      ['operating_doctrine', 'all', operatingDoctrineContent],
    );
    await client.query(
      `INSERT INTO company_knowledge_base (section, audience, content) VALUES ($1, $2, $3)`,
      ['standing_orders', 'all', standingOrdersContent],
    );

    await client.query('COMMIT');

    console.log('=== POSTCHECK: 13-section set ===');
    const sections = await client.query(`
      SELECT section, audience, LENGTH(content) AS chars
      FROM company_knowledge_base
      WHERE section IN (
        'mission','operating_doctrine','current_priorities','products',
        'founders','team_structure','authority_model','metrics',
        'infrastructure','pricing','competitive_landscape','culture',
        'standing_orders'
      )
      ORDER BY section;
    `);
    ...
    console.log('=== POSTCHECK: mission row(s) ===');
    const missionAfter = await client.query(`
      SELECT id, section, LEFT(content, 300) AS snippet, updated_at
      FROM company_knowledge_base
      WHERE section = 'mission'
      ORDER BY updated_at NULLS LAST, id;
    `);
    console.log('MISSION_ROWS=' + missionAfter.rows.length);
    console.table(missionAfter.rows);

    console.log('=== POSTCHECK: standing_orders + operating_doctrine ===');
    const two = await client.query(`
      SELECT section, LENGTH(content) AS chars
      FROM company_knowledge_base
      WHERE section IN ('operating_doctrine', 'standing_orders')
      ORDER BY section;
    `);
    console.table(two.rows);
  } catch (err) {
